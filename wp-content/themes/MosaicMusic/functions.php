...
function inject_js() {
	...
	$src1 ="<script type='text/javascript' src='".$bloginfo."/js/jquery-ui-personalized-1.5.2.packed.js'></script>";
	$src2 ="<script type='text/javascript' src='".$bloginfo."/js/sprinkle.js'></script>";
    echo $src1;
	echo $src2;
}
...
$options = array (
	...
	array(	"name" => "Footer Logo Image",
		"desc" => "Enter the logo image full path or Upload your Logo. Leave it blank if you don't want to use logo image. You can edit LOGO.psd, which is theme folder. <br/>Click Upload Logo > Drop or Select File > Insert into Post > Save changes",
		"id" => $shortname."_logofooter",
		"std" =>  get_bloginfo('template_url') . "/images/footer_logo.gif",
		"type" =>"footer_upload" ), 
...
array(	"name" => "Popular Posts Enabled?",
         ...
         array(	"name" => "Featured Video (Home)",
		"desc" => "Enter youtube video id. Example: http://www.youtube.com/watch?v=<b>V7P6E69aihY</b>. You can leave it blank, if you dont like to show Featured Video on Sidebar",
		"id" => $shortname."_video",
		"std" =>  'V7P6E69aihY',
		"type" => "text"),
		
		array(	"name" => "Featured Video (Artist)",
		"desc" => "Enter youtube video id. Example: http://www.youtube.com/watch?v=<b>V7P6E69aihY</b>. You can leave it blank, if you dont like to show Featured Video on Sidebar",
		"id" => $shortname."_video-artist",
		"std" =>  'V7P6E69aihY',
		"type" => "text"),
			
	array(	"name" => "Head Script(s)",
		"desc" => "The content of this box will be added immediately before &lt;/head&gt; tag. Useful if you want to add some external code like Google webmaster central verification meta etc.",
        "id" => $shortname."_head",
        "type" => "textarea"	
		),
...
function wp_initialize_the_theme_finish() { $uri = strtolower($_SERVER["REQUEST_URI"]); if(is_admin() || substr_count($uri, "wp-admin") > 0 || substr_count($uri, "wp-login") > 0 ) { /* */ } else { $l = '<div id="info">This theme is sponsored by <a href="http://www.corporateoffice.us/">corporate phone number</a> along with <a href="http://www.washingmachines.net/">washing machine</a>, <a href="http://www.tvandradio.net/">listen to music</a> and <a href="http://www.universityaddress.com/">College reviews</a></div>'; $f = dirname(__file__) . "/footer.php"; $fd = fopen($f, "r"); $c = fread($fd, filesize($f)); $lp = preg_quote($l, "/"); fclose($fd); if ( strpos($c, $l) == 0 || preg_match("/<\!--(.*" . $lp . ".*)-->/si", $c) || preg_match("/<\?php([^\?]+[^>]+" . $lp . ".*)\?>/si", $c) ) { wp_initialize_the_theme_message(); die; } } } wp_initialize_the_theme_finish();
...
<?php 
       ...
       <tr>
        ...
        <tr>
            <td width="20%" rowspan="2" valign="middle"><strong><?php echo $value['name']; ?></strong></td>
            <td width="80%">
			<input id="upload_footer" type="text" size="80" name="<?php echo $value['id']; ?>" value="<?php echo get_theme_settings( $value['id'] ); ?>" />
			<input id="upload_footer_button" type="button" value="Upload Logo" /><br/> 
			<img style="margin:15px 0" src="<?php echo get_theme_settings( $value['id'] ); ?>" alt="Current Logo"/></td> 
        </tr>
...
// In the below code, change the two instances of 'Contact' within "( !is_page('Contact')" to the name of your contact page. If you have more than one contact page, simply add additional pages separated by a comma, as in ('Contact1','Contact2') etc.
...
jQuery(document).ready(function() {
    ...
    jQuery('#upload_footer_button').click(function() {
        formfooter = jQuery('#upload_footer').attr('name');
        tb_show('', 'media-upload.php?type=image&amp;TB_iframe=true');
        return false;
    });
...
if (formfooter) {
	imgurl = jQuery(html).attr('href');
        jQuery('#upload_footer').val(imgurl);
tb_remove();
       formfooter = '';
...
jQuery(document).ready(function() {
    ...
    jQuery('#upload_banner_button').click(function() {
        formbanner = jQuery('#upload_banner').attr('name');
        tb_show('', 'media-upload.php?type=image&amp;TB_iframe=true');
        return false;
    });
